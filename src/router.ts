...
router
  ...
  .patch("/patients/:id", async (req: SuGoRequest, res: SuGoResponse) =>
    handleResponse(res, 200, await service.patchPatient(req.params.id, req.body.firstName, req.body.lastName))
  )
  .delete("/patients/:id", async (req: SuGoRequest, res: SuGoResponse) => handleResponse(res, 200, await service.deletePatient(req.params.id)))
